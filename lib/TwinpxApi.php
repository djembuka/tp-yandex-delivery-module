...
class TwinpxApi
{
    ...
    public static function PrepareData($arData, $order_id = FALSE, $isAdmin = FALSE)
    {
		...
		//сохраняем сессий
		$session->set('FULL_ADDRESS', $full_address);//передаем полный адрес в сессию, для записи в БД
		$session->set('DELIVERY_COST', $delivery_cost); //передаем delivery_cost в сохранение заказа
    ...
    public static function GetItems($order_id = FALSE)
    {
        ...
        if($order_id > 0) { //если передан ID заказа, получаем список товароа
        ...
        foreach ($basket as $basket_item) {
            ...
            if($product['CAN_BUY'] == 'Y') {  //только доступные товары к покупке
            	$arData[] = $product;
	...
	//            $dz		= $default['Width']; //mm
	            ...
	            //обем товара
	            $arSizes = unserialize($data['DIMENSIONS']); //в мм
    ...
    function GetPlaceBarcode($order_id = false, $len = 30)
    {
        ...
        if(strlen($strBarcode) > ($len - strlen($order) + 1)) { //если префикс более 29 символов
        	if($order > 0) { //если имеем номер заказа
				...
				$barcode = $strBarcode; //если нет № закза просто возврящаем урезанный префикс
			}
		}
        ...
        $session->set("YDELIVERY_BARCODE", $return); //запоминаем в сессию
...
//        $tz = new \DateTimeZone('Europe/Moscow'); // or whatever zone you're after
	...
	*/
    public static function requestPost($uri, $data = array(), $oAuth = false)
    {
		...
		$oAuth = ($oAuth) ? $oAuth : TwinpxApi::GetApiOAuth(); //если передан $oAuth вручную
